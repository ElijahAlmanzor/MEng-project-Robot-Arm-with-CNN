...
def distort_images(image, image_depth, height, width, thread_id):
    ...
    #rgbd_image = tf
    #rgbd_image = tf.image.random_flip_left_right(rgbd_image) #surely bounding box would be wrong?
    return rgbd_image
...
def batch_inputs(data_files, train, num_epochs, batch_size,
    ...
    if train:
        ...
        filename_queue = tf.train.string_input_producer(data_files,
                                                        num_epochs,
                                                        shuffle=True,
                                                        capacity=16) #has the epoch converted into the steps look at how this relates to the whole threading and stuff
        
        '''string_input_producer which take a list of filenames and creates a FIFOQueue if shuffle is off and RandomShuffleQ if the shuffle=True'''
    ...
    if train:
        ...
        examples_queue = tf.RandomShuffleQueue(
            capacity=min_queue_examples+3*batch_size, #'''this is the capacity of the random Q generated''''
            min_after_dequeue=min_queue_examples,  #this is just the minimum elements in queue until it forces to add more elements
            dtypes=[tf.string])
        ''' When want to extract  randomized batch data from a a queue, want ot make sure all the data is truly randomised. 
        RandomShuffleQueue is very similar to FIFOQueue, except for first in, first out, it just dequeues in a random manner
        min_after_dequeue --> if after a deqeueing op, the number of examples in the queue falls below this value, it will block further dequeing until more samples are added to the queue
        this essentially forces an enqeueing operation. '''
    ...
    if num_readers > 1:
        enqueue_ops = []
        for _ in range(num_readers): #the threads for the queuing and the dueuing
            reader = tf.TFRecordReader()
            _, value = reader.read(filename_queue) #so this deQ's from filename_queue
            enqueue_ops.append(examples_queue.enqueue([value])) #it then adds it on to the example Q
        tf.train.queue_runner.add_queue_runner(
            tf.train.queue_runner.QueueRunner(examples_queue,enqueue_ops))
        examples_serialized = examples_queue.dequeue()
        '''Notes about readers
        Reads the binary files but also implicitly handles the dequeueing operation from the Q'''
...
def distorted_inputs(data_files, num_epochs, train=True, batch_size=None):
    with tf.device('/cpu:0'): #could be a GPU? something about not a valid device lol, or could increase the number of threads to increase training speed?
        print(train)
